...
struct AvbOps {
	...
	/* Gets the starting pointer of a partition that is pre-loaded in
	 * memory, and save it to |out_pointer|. The preloaded partition is
	 * expected to be |num_bytes|, where the actual preloaded byte count is
	 * returned in |out_num_bytes_preloaded|. |out_num_bytes_preloaded|
	 * must be no larger than |num_bytes|.
	 *
	 * This provides an alternative way to access a partition that is
	 * preloaded into memory without a full memory copy. When this function
	 * pointer is not set (has value NULL), or when the |out_pointer| is
	 * set to NULL as a result, |read_from_partition| will be used as the
	 * fallback. This function is mainly used for accessing the entire
	 * partition content to calculate its hash.
	 *
	...
	AvbIOResult (*get_preloaded_partition)(AvbOps *ops,
			const char *partition, size_t num_bytes,
	...
	/* Sets whether the device is locked or unlocked. Returns
	 * AVB_IO_RESULT_OK
	 * if the state was set, otherwise an error code.
	 */
	AvbIOResult (*write_device_lock_state)(AvbOps *ops,
					       int state);
	 ...
	 * |out_size_num_bytes|.
	 *
	 * If the partition doesn't exist the
	 * AVB_IO_RESULT_ERROR_NO_SUCH_PARTITION
	...
	/* Reads a persistent value corresponding to the given |name|. The
	 * value is returned in |out_buffer| which must point to |buffer_size|
	 * bytes. On success |out_num_bytes_read| contains the number of bytes
	 * read into |out_buffer|. If AVB_IO_RESULT_ERROR_INSUFFICIENT_SPACE is
	 * returned, |out_num_bytes_read| contains the number of bytes that
	 * would have been read which can be used to allocate a buffer.
	 *
	 * The |buffer_size| may be zero and the |out_buffer| may be NULL, but
	 * if |out_buffer| is NULL then |buffer_size| *must* be zero.
	 *
	 * Returns AVB_IO_RESULT_OK on success, otherwise an error code.
	 *
	 * If the value does not exist, is not supported, or is not populated,
	 * returns AVB_IO_RESULT_ERROR_NO_SUCH_VALUE. If |buffer_size| is
	 * smaller than the size of the stored value, returns
	 * AVB_IO_RESULT_ERROR_INSUFFICIENT_SPACE.
	 *
	 * This operation is currently only used to support persistent digests
	 * or the AVB_HASHTREE_ERROR_MODE_MANAGED_RESTART_AND_EIO hashtree
	 * error mode. If a device does not use one of these features this
	...
	/* Writes a persistent value corresponding to the given |name|. The
	 * value is supplied in |value| which must point to |value_size| bytes.
	 * Any existing value with the same name is overwritten. If |value_size|
	 * is zero, future calls to |read_persistent_value| will return
	 * AVB_IO_RESULT_ERROR_NO_SUCH_VALUE.
	 *
	 * Returns AVB_IO_RESULT_OK on success, otherwise an error code.
	 *
	 * If the value |name| is not supported, returns
	 * AVB_IO_RESULT_ERROR_NO_SUCH_VALUE. If the |value_size| is not
	 * supported, returns AVB_IO_RESULT_ERROR_INVALID_VALUE_SIZE.
	 *
	 * This operation is currently only used to support persistent digests
	 * or the AVB_HASHTREE_ERROR_MODE_MANAGED_RESTART_AND_EIO hashtree error
	 * mode. If a device does not use one of these features this function
	 ...
	 * AVB_SLOT_VERIFY_FLAGS_NO_VBMETA_PARTITION is being used. The name of
	 * the partition to get the public key for is passed in
	 * |partition_name|.
	 *
