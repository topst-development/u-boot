		      ...
		      const AvbCmdlineSubstList *additional_substitutions)
{
	...
	const char *replace_str[NUM_GUIDS] = {
	...
	for (n = 0; n < NUM_GUIDS; n++) {
		...
		/* Don't attempt to query the partition guid unless its search
		 * string is present in the command line. Note: the original
		 * cmdline is used here, not the replaced one. See b/116010959.
		 */
		...
		if (ret == NULL) {
			...
			char *new_ret = avb_replace(ret, replace_str[n],
						    guid_buf);
			avb_free(ret);
			ret = new_ret;
		}
				  ...
				  uint64_t minor_version)
{
	...
	avb_memcpy(combined + num_major_digits + 1, minor_digits,
		   num_minor_digits);
		...
		AvbHashtreeErrorMode resolved_hashtree_error_mode)
{
	...
	case AVB_ALGORITHM_TYPE_SHA512_RSA8192: {
		...
		avb_slot_verify_data_calculate_vbmeta_digest(
				slot_data, AVB_DIGEST_TYPE_SHA512,
				vbmeta_digest);
		for (n = 0; n < slot_data->num_vbmeta_images; n++)
			total_size += slot_data->vbmeta_images[n].vbmeta_size;
		if (!cmdline_append_option(slot_data,
	...
	/* Set androidboot.veritymode and
	...
	if (toplevel_vbmeta->flags & AVB_VBMETA_IMAGE_FLAGS_HASHTREE_DISABLED) {
		...
		case AVB_HASHTREE_ERROR_MODE_EIO:
			 ...
			 * just use 'ignore_zero_blocks' since that's already
			 * set and dm-verity-target.c supports specifying this
			 * multiple times.
			 */
		...
		new_ret = avb_replace(slot_data->cmdline,
				      "$(ANDROID_VERITY_MODE)", dm_verity_mode);
		avb_free(slot_data->cmdline);
	...
	if (hashtree_error_mode ==
		...
		if (!cmdline_append_option(
				slot_data, "androidboot.veritymode.managed",
				"yes")) {
			ret = AVB_SLOT_VERIFY_RESULT_ERROR_OOM;
			goto out;
		}
		...
		AvbCmdlineSubstList *out_cmdline_subst)
{
	...
	if (out_cmdline_subst->size >= AVB_MAX_NUM_CMDLINE_SUBST) {
		/* The list is full. Currently dynamic growth of this list is
		 * not supported.
		 */
		return AVB_SLOT_VERIFY_RESULT_ERROR_INVALID_METADATA;
	}

	/* Construct the token to replace in the command line based on the
	 * partition name. For partition 'foo', this will be
	 * '$(AVB_FOO_ROOT_DIGEST)'.
	 */
	out_cmdline_subst->tokens[list_index] =
			avb_strdupv(kDigestSubPrefix, part_name,
				    kDigestSubSuffix, NULL);
	...
	out_cmdline_subst->values[list_index] =
			avb_bin2hex(digest, digest_size);
