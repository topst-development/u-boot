...
static void clock_init_main(void)
{
	const struct clkctrl clk_list[17] = {
		...
		{FBUS_CODA,	585000000UL, 0UL}, /* Clock source Duty ratio should be 50:50 */
		{FBUS_CHEVCDEC,	705000000UL, 0UL},
		{FBUS_BHEVCDEC,	705000000UL, 0UL},
		{FBUS_CHEVCENC,1170000000UL, 0UL},
		{FBUS_BHEVCENC,	940000000UL, 0UL},
		{FBUS_VBUS,	940000000UL, 0UL},

		{FBUS_AUDIO,	400000000UL, 0UL}, /* 398.131... MHz */

		{FBUS_CAM_APB,	200000000UL, 0UL},
		{FBUS_CAM_AXI,	800000000UL, 0UL},
		{FBUS_CAM_PIX,	800000000UL, 0UL},
		{FBUS_CAM_SYS,	400000000UL, 0UL},
		{PERI_CAM_I2C,	 50000000UL, 0UL}
	...
	(void)tcc_set_pll(PLL_VIDEO_0, CKC_ENABLE, 2820000000UL, 0);
	(void)tcc_set_pll_div(PLL_VIDEO_0_DIV_0, 3); //  940 MHz
	(void)tcc_set_pll_div(PLL_VIDEO_0_DIV_1, 4); //  705 MHz
	(void)tcc_set_pll(PLL_VIDEO_1, CKC_ENABLE, 2340000000UL, 0);
	(void)tcc_set_pll_div(PLL_VIDEO_1_DIV_0, 2); // 1170 MHz
	(void)tcc_set_pll_div(PLL_VIDEO_1_DIV_1, 4); //  585 MHz
	...
	(void)tcc_set_pll(PLL_DP, CKC_ENABLE, 800000000UL, 0);
	(void)tcc_set_pll_div(PLL_DP_DIV_0, 2); // 400 MHz
	(void)tcc_set_pll_div(PLL_DP_DIV_1, 50);//  16 MHz
	(void)tcc_set_pll_div(PLL_DP_DIV_2, 4); // 200 MHz
	...
	(void)tcc_set_clkctrl(PERI_CAM_I2C,	CKC_ENABLE, XIN_CLK_RATE/2UL, CLK_F_SRC_CLK(CAM_CLKCTRL_SEL_XIN));

	(void)tcc_set_pll(PLL_CAM, CKC_ENABLE, 1600000000UL, 0);
	(void)tcc_set_pll_div(PLL_CAM_DIV_0, 8); // APB 200 MHz
	(void)tcc_set_pll_div(PLL_CAM_DIV_1, 2); // AXI 800 MHz
	(void)tcc_set_pll_div(PLL_CAM_DIV_2, 4); // SYS 400 MHz
	(void)tcc_set_pll_div(PLL_CAM_DIV_3, 2); // PIX 800 MHz
	(void)tcc_set_pll_div(PLL_CAM_DIV_4, 32);// I2C  50 MHz

	/* AUDIO CLOCK */
	(void)tcc_set_clkctrl(FBUS_AUDIO, CKC_ENABLE, XIN_CLK_RATE/2UL, CLK_F_SRC_CLK(AUDIO_CLKCTRL_SEL_XIN));

	(void)tcc_set_pll(PLL_AUDIO_0, CKC_ENABLE, 1990656000UL, 0UL);
	(void)tcc_set_pll_div(PLL_AUDIO_0_DIV, 5);
	(void)tcc_set_pll(PLL_AUDIO_1, CKC_ENABLE, 2099865600UL, 0UL);
	(void)tcc_set_pll_div(PLL_AUDIO_1_DIV, 6);
...
void clock_init_early(void)
{
	...
	if (is_main_core(binfo.coreid)) {
		...
		(void)tcc_set_clkctrl(FBUS_MP_CA76_0,  CKC_ENABLE, 1700000000UL, 0UL);
		(void)tcc_set_clkctrl(FBUS_MP_CA76_1,  CKC_ENABLE, 1700000000UL, 0UL);
		(void)tcc_set_clkctrl(FBUS_MP_CA76_2,  CKC_ENABLE, 1700000000UL, 0UL);
		(void)tcc_set_clkctrl(FBUS_MP_CA76_3,  CKC_ENABLE, 1700000000UL, 0UL);

		(void)tcc_set_clkctrl(FBUS_MP_CA55_0,  CKC_ENABLE, 1500000000UL, 0UL);
		(void)tcc_set_clkctrl(FBUS_MP_CA55_1,  CKC_ENABLE, 1500000000UL, 0UL);
		(void)tcc_set_clkctrl(FBUS_MP_CA55_2,  CKC_ENABLE, 1500000000UL, 0UL);
		(void)tcc_set_clkctrl(FBUS_MP_CA55_3,  CKC_ENABLE, 1500000000UL, 0UL);

		if (!core_reset_occurred()) {
			clock_init_early_main();
			if (is_dual_boot(binfo.bootsel)) {
				arm_smccc_smc(SIP_CLK_SYNC, 0, true, 0, 0, 0, 0, 0, &res);
			}
		}
	} else {
		(void)tcc_set_clkctrl(FBUS_SP_CA55_0,  CKC_ENABLE, 1500000000UL, 0UL);
		(void)tcc_set_clkctrl(FBUS_SP_CA55_1,  CKC_ENABLE, 1500000000UL, 0UL);

		if (!core_reset_occurred()) {
			do {
				arm_smccc_smc(SIP_CLK_SYNC, 0, false, 0, 0, 0, 0, 0, &res);
			} while (res.a3 != CKC_OK);
