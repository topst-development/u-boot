...
#define VPLL_DIV(div1, div2)  ((unsigned long)((unsigned long)(div2) << 8) | (unsigned long)(div1))
...
void clock_init(void)
{
        ...
        /* FBUS_IO is already set in clock_init_early() */
        struct clkctrl initial_clkctrl[16] = {
                {FBUS_CPUBUS,   400000000},
                {FBUS_CMBUS,    0},
        ...
        for (i = 0; i < ARRAY_SIZE(initial_clkctrl); i++) {
                ...
                if(((name == 0x8059U) || (name == 0x8053U)) &&
                                (ctrl->channel == FBUS_GPU)) {
                       ctrl->rate = 420000000;
                }

                if(((name == 0x8050U) && (rev ==0x2U)) &&
                                (ctrl->channel == FBUS_GPU)) {
                       ctrl->rate = 700000000;
                }

                if (ctrl->rate != 0) {
                        (void)tcc_set_clkctrl(ctrl->channel, CKC_ENABLE, ctrl->rate, ctrl->flags);
                }
        }
#endif
}
...
void clock_init_early(void)
{
         ...
         */
        (void)tcc_set_pll(PLL_VIDEO_0, CKC_ENABLE, 3120000000UL, 0);
