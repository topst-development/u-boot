	...
	void __iomem *reg, unsigned int layer, unsigned int mode)
{
	...
	if (layer > 2U) {
		...
		val = (__raw_readl(reg + (MROPC0 + (0x10U * layer)))
			& ~(MROPC_ROPMODE_MASK));
		val |= ((mode & 0x1FU) << MROPC_ROPMODE_SHIFT);
		__raw_writel(val, reg + (MROPC0 + (0x10U * layer)));
	}
}
