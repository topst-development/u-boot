...
static int debug_wmix;
#define dprintk(msg...)								\
	do {											\
		if (debug_wmix == 1) {						\
			(void)pr_info("[DBG][VIOC_WMIX] " msg);	\
		}											\
	} while ((bool)0)
	...
	unsigned int vioc_id, unsigned int image_num)
{
	...
	int layer;
	int mixer_type;
	void __iomem *reg = VIOC_WMIX_GetAddress(vioc_id);
	int ret = -1;
...
void __iomem *VIOC_WMIX_GetAddress(unsigned int vioc_id)
{
	...
	if (Num == 0xFFU) {
		dprintk("%s: vioc_id is 0x%x\n", __func__, vioc_id);
		ret = NULL;
	} else if (Num >= VIOC_WMIX_MAX) {
