...
defined(CONFIG_TCC901X) || defined(CONFIG_TCC805X) || defined(CONFIG_TCC750X)
...
void vioc_intr_initialize(void)
{
...
#endif
#if defined(CONFIG_TCC750X)
	__raw_writel(0xffffffff, reg + IRQMASKCLR0_3_OFFSET);
#endif
	...
	for (i = 0; i <= ((unsigned int)VIOC_INTR_DEV0
			- (unsigned int)VIOC_INTR_DEV0); i++) {
#endif
#endif
...
#else //defined(CONFIG_TCC750X)
	for (i = 0; i <= ((unsigned int)VIOC_INTR_RD6
			- (unsigned int)VIOC_INTR_RD0); i++) {
#endif
...
#else //defined(CONFIG_TCC750X)
	for (i = 0; i <= ((unsigned int)VIOC_INTR_WD3 - (unsigned int)VIOC_INTR_WD0); i++) {
#endif
		reg = VIOC_WDMA_GetAddress(i);
		if (reg == NULL) {
			/* Prevent KCS warning*/
