...
static enum command_ret_t sub_do_common(bool arg_chk, const char *partname,
					struct blk_desc **desc,
					struct disk_partition *info, s32 *part)
{
	enum command_ret_t ret = CMD_RET_SUCCESS;
		      ...
		      char *const argv[])
{
	...
	ret = sub_do_common(arg_chk, argv[1], &desc, &info, &part);

	if (ret == CMD_RET_SUCCESS) {
		s32 err = strict_strtoul(argv[2], 16, &addr);
		       ...
		       char *const argv[])
{
	...
	if (ret == CMD_RET_SUCCESS) {
		...
		if ((ret == CMD_RET_SUCCESS) && (argc == 4)) {
			size *= info.blksz;
			(void)env_set_hex(argv[3], size);
		}
	}
...
#if defined(CONFIG_TCC_DMMETA_IMAGE)
static int sub_do_dmmeta(struct cmd_tbl *cmd, int flags, int argc,
		       char *const argv[])
{
	enum command_ret_t ret = CMD_RET_SUCCESS;
	bool arg_chk = ((cmd->maxargs > 0) && (argc >= (cmd->maxargs - 1)));
	...
	struct dmmeta meta;
	uint64_t base;
	uint32_t blkstart, blkcount;
	char *buf;

	ret = sub_do_common(arg_chk, argv[1], &desc, &info, &part);

	if (ret == CMD_RET_SUCCESS) {
		buf = dma_alloc_coherent(4096, &addr);
		if (buf == NULL) {
			ret = CMD_RET_FAILURE;
		}
	}
	...
	if (ret == CMD_RET_SUCCESS) {
		/* Read super block */
		blkstart = info.start;
		blkcount = BLOCK_CNT(4096, desc);
		if (blk_dread(desc, blkstart, blkcount, buf) == blkcount) {
			memcpy(&sb, &buf[1024], sizeof(sb));
		} else {
			pr_err("Read fail\n");
			ret = CMD_RET_FAILURE;
		}
	...
	if (ret == CMD_RET_SUCCESS) {
		/* Read DM-Meta */
		base = sb.total_blocks << (10 + sb.log2_block_size);
		blkstart = info.start + BLOCK_CNT(base, desc);
		blkcount = 1;
		if (blk_dread(desc, blkstart, blkcount, buf) == blkcount) {
			memcpy(&meta, buf, sizeof(struct dmmeta));
		} else {
			pr_err("Read fail\n");
			ret = CMD_RET_FAILURE;
		}
	...
	if (ret == CMD_RET_SUCCESS) {
		/* Get signature */
		base = base + dmmeta_size(&meta);
		blkstart = info.start + BLOCK_CNT(base, desc);
		blkcount = 1;
		if (blk_dread(desc, blkstart, blkcount, buf) == blkcount) {
			/* DM Verity Setup */
			ret = dmmeta_setup(argv[2], &meta, buf);
		} else {
			pr_err("Read fail\n");
			ret = CMD_RET_FAILURE;
		}
		       ...
		       char *const argv[])
{
	...
	if ((ret == CMD_RET_SUCCESS) && (desc->devnum >= 0) && (part >= 0)) {
		(void)env_set_ulong(argv[2], (ulong)desc->devnum);
		(void)env_set_ulong(argv[3], (ulong)part);
	}
...
U_BOOT_CMD_WITH_SUBCMDS(
	...
	"blkread image <part> <addr>\n"
	"    - read a boot image from the partition `part' to memory\n"
	"      address `addr'\n"

	"blkread fdt <part> <addr>\n"
	"    - read a FDT image from the partition `part' to memory\n"
	"      address `addr'\n"

	"blkread part <part> <addr> [size_var]\n"
	"    - read a partition `part' to memory address `addr' and\n"
	"      return loaded amounts (in byte) in `size_var' if given\n"

#if defined(CONFIG_TCC_DMMETA_IMAGE)
	"blkread dmmeta <part> <dev_path>\n"
	"    - read a partition `part` in `slot and set `dmsetup` env\n"
#endif
	"blkread info <part> <devnum_var> <partnum_var>\n"
	"    - get device number and partition number of a partition\n"
	"      `part' and store each in `devnum_var' and `partnum_var'\n",

	/* sub-command entry list */
	U_BOOT_SUBCMD_MKENT(dev, 3, 0, sub_do_dev),
	U_BOOT_SUBCMD_MKENT(image, 3, 0, sub_do_img),
	U_BOOT_SUBCMD_MKENT(fdt, 3, 0, sub_do_fdt),
	U_BOOT_SUBCMD_MKENT(part, 4, 0, sub_do_part),
#if defined(CONFIG_TCC_DMMETA_IMAGE)
	U_BOOT_SUBCMD_MKENT(dmmeta, 3, 0, sub_do_dmmeta),
#endif
	U_BOOT_SUBCMD_MKENT(info, 4, 0, sub_do_info),
);
