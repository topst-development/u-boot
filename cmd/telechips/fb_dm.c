		...
		if (fb_id == CMD_FB_SHOW_ALL) {
			...
			if(fb_id == priv->id) {
				...
				*/
				...
				printf("  %-12s= %d\n", "lcd_disp[disp]", get_vioc_index(priv->lcd_disp));
				printf("  %-12s= %d\n", "clk_div", priv->clk_div);
				printf("  %-12s= %d\n", "ovp", priv->ovp);
				printf("  %-12s= %s\n", "fb draw type",
						(priv->fb_draw_type == 0U) ? "bitmap" : 
						(priv->fb_draw_type == 1U) ? "splash" : 
						"disable");
...
static void blen_fb_dm(int fb_id)
{
	struct tcc_fb_dm_priv *priv;
	struct udevice *dev;
	struct uclass *uc;
	...
	uclass_id_foreach_dev(UCLASS_VIDEO, dev, uc) {
		if (device_active(dev)) {
			...
			if (priv != NULL) {
				if ((fb_id == CMD_FB_SHOW_ALL) || (fb_id == priv->id)) {
					video_backlight(dev, (bool)true);
				}
			}
		}
	}
