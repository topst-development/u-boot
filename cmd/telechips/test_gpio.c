...
#define GPIO_MULTI_CPU0			0
#define GPIO_MULTI_CPU1			1
#define GPIO_MULTI_CPU2			2
#define GPIO_MULTI_CPU3			3
#define GPIO_MULTI_CPU_BASE		(0x800)
#define GPIO_MULTI_CPU_MAX_NUM	34
#define GPIO_MULTI_GPIOA	 	0
#define GPIO_MULTI_GPIOA_2	 	1
#define GPIO_MULTI_GPIOB 		2
#define GPIO_MULTI_GPIOB_2 		3
#define GPIO_MULTI_GPIOC 		4
#define GPIO_MULTI_GPIOC_2 		5
//#define GPIO_MULTI_GPIOD 		6	//not exist
//#define GPIO_MULTI_GPIOD_2 	7	//not exist
#define GPIO_MULTI_GPIOE 		8
#define GPIO_MULTI_GPIOE_2 		9
//#define GPIO_MULTI_GPIOF 		10	//not exist
//#define GPIO_MULTI_GPIOF_2 	11	//not exist
#define GPIO_MULTI_GPIOG 		12
#define GPIO_MULTI_GPIOG_2 		13
//#define GPIO_MULTI_RESERVED1	14
//#define GPIO_MULTI_RESERVED2	15
#define GPIO_MULTI_GPIOSD0		16
#define GPIO_MULTI_GPIOSD0_2	17
#define GPIO_MULTI_GPIOSD1		18
#define GPIO_MULTI_GPIOSD1_2	19
#define GPIO_MULTI_GPIOSD2		20
#define GPIO_MULTI_GPIOSD2_2	21
#define GPIO_MULTI_GPIOH		22
#define GPIO_MULTI_GPIOH_2		23
#define GPIO_MULTI_GPIOK		24
#define GPIO_MULTI_GPIOK_2		25
#define GPIO_MULTI_GPIOMA		26
#define GPIO_MULTI_GPIOMA_2		27
#define GPIO_MULTI_GPIOMB		28
#define GPIO_MULTI_GPIOMB_2		29
#define GPIO_MULTI_GPIOMC		30
#define GPIO_MULTI_GPIOMC_2		31
#define GPIO_MULTI_GPIOMD		32
#define GPIO_MULTI_GPIOMD_2		33
...
static void test_rw(struct udevice *dev, const char *bank_name, s32 pin_num, s32 cpu_num);
	...
	(u32 addr, u32 pin_num, u32 width)
{
	...
	reg_data = readl((unsigned long int)addr);
		...
		u32 pin_num, u32 value, u32 offset)
{
	...
	if (value == 1) {
		writel(readl((unsigned long int)addr) | BIT(pin_num)
	...
	u32 pin_num, u32 offset)
{
	...
	return !!(readl((unsigned long int)addr) & BIT(pin_num));
}
...
void test_rw(struct udevice *dev,
		const char *bank_name, s32 pin_num, s32 cpu_num)
{
	s32 i;
	s32 j;
	u32 addr;
	...
	for (i = 0; i < priv->nbanks; i++) {
		if (strncmp(priv->bank[i].name, bank_name, strlen(bank_name))
			...
			addr = priv->gpio_base + (0x1000 * (cpu_num+1))
				+ priv->bank[i].bank_base;
#else
...
void test_rw_all(struct udevice *dev)
{
	...
	TCC_DEBUG_MSG("priv->nbanks : %d\n", priv->nbanks);
	for (i = 0; i < priv->nbanks; i++) {
		addr = priv->gpio_base + priv->bank[i].bank_base;
		TCC_DEBUG_MSG("addr : 0x%X\n", addr);
...
static s32 test_or_register(u32 addr, s32 pin_num)
{
	...
	if (expect_value != read_value) {
		TCC_DEBUG_MSG(
			"Failed test - expect value : %d, real value : %d\n"
...
static s32 test_eclk(u32 addr)
{
	...
	for (eclk_num = 0; eclk_num < 4; eclk_num++) {
		...
		TCC_DEBUG_MSG(
				"external External Input Clock number : %d\n"
				, eclk_num);
...
static s32 test_eint(u32 addr)
{
	...
	for (eint_num = 0; eint_num < 8; eint_num++) {
		backup_value = test_gpio_get_eint(addr, eint_num);

		TCC_DEBUG_MSG("external interrupt number : %d\n", eint_num);
		for (expect_value = 0; expect_value < 255; expect_value++) {
...
static s32 test_gpio_get_eint(u32 bank_base, u32 eint_num)
{
	...
	u32 value = readl((unsigned long int)eint_addr);
		...
		u32 bank_base, u32 eint_num, u32 eint_source)
{
	...
	writel(value, (unsigned long int)eint_addr);
		...
		u32 bank_base, u32 eclk_num)
{
	...
	u32 value = readl((unsigned long int)eclk_addr);
		...
		u32 bank_base, u32 eclk_num, u32 eint_source)
{
	...
	writel(value, (unsigned long int)eclk_addr);
...
static s32 test_gpio_get_function(u32 bank_base, u32 pin_num)
{
	...
	u32 value = readl((unsigned long int)func_addr);
...
static s32 test_gpio_set_function(u32 bank_base, u32 pin_num, u32 func)
{
	...
	writel(value, (unsigned long int)func_addr);
...
static s32 test_gpio_get_drive_strength(u32 bank_base, u32 pin_num)
{
	...
	u32 value = readl((unsigned long int)drive_strength_addr);
		...
		u32 bank_base, u32 pin_num, u32 drive_strength)
{
	...
	writel(value, (unsigned long int)drive_strength_addr);
...
static s32 test_gpio_get_pull_enable(u32 bank_base, u32 pin_num)
{
	...
	return !!(readl((unsigned long int)pull_enable_addr) & BIT(pin_num));
}
...
static s32 test_gpio_set_pull_enable(u32 bank_base, u32 pin_num, u32 pe)
{
	...
	if (pe == 1) {	//pull enable
		writel(
			readl((unsigned long int)pull_enable_addr) | BIT(pin_num)
			, (unsigned long int)pull_enable_addr);
	} else {		//pull disable
		writel(
			readl((unsigned long int)pull_enable_addr) & (~BIT(pin_num))
			, (unsigned long int)pull_enable_addr);
...
static s32 test_gpio_get_pull_select(u32 bank_base, u32 pin_num)
{
	...
	return !!(readl((unsigned long int)pull_select_addr) & BIT(pin_num));
}
...
static s32 test_gpio_set_pull_select(u32 bank_base, u32 pin_num, u32 ps)
{
	...
	if (ps == 1) {	//pull up
		writel(
			readl((unsigned long int)pull_select_addr) | BIT(pin_num)
			, (unsigned long int)pull_select_addr);
	} else {		//pull down
		writel(
			readl((unsigned long int)pull_select_addr) & (~BIT(pin_num))
			, (unsigned long int)pull_select_addr);
...
static s32 test_gpio_get_output_enable(u32 bank_base, u32 pin_num)
{
	...
	return !!(readl((unsigned long int)oen_addr) & BIT(pin_num));
}
...
static s32 test_gpio_set_output_enable(u32 bank_base, u32 pin_num, u32 oen)
{
	...
	if (oen == 1) {	//output enable
		writel(readl((unsigned long int)oen_addr) | BIT(pin_num), (unsigned long int)oen_addr);
		/* comment for kernel coding style */
	} else {		//output disable
		writel(readl((unsigned long int)oen_addr) & (~BIT(pin_num)), (unsigned long int)oen_addr);
		/* comment for kernel coding style */
...
static s32 test_gpio_get_data(u32 bank_base, u32 pin_num)
{
	...
	return !!(readl((unsigned long int)data_addr) & BIT(pin_num));
}
...
static s32 test_gpio_set_or_reg(u32 bank_base, u32 pin_num)
{
	...
	writel(readl((unsigned long int)data_addr) | BIT(pin_num), (unsigned long int)data_addr);
...
static s32 test_gpio_get_input_enable(u32 bank_base, u32 pin_num)
{
	...
	return !!(readl((unsigned long int)ien_addr) & BIT(pin_num));
}
		...
		u32 bank_base, u32 pin_num, u32 ien)
{
	...
	if (ien == 1) {	//input enable
		writel(readl((unsigned long int)ien_addr) | BIT(pin_num), (unsigned long int)ien_addr);
		/* comment for kernel coding style */
	} else {		//input disable
		writel(readl((unsigned long int)ien_addr) & (~BIT(pin_num)), (unsigned long int)ien_addr);
		/* comment for kernel coding style */
...
static s32 test_gpio_get_input_type(u32 bank_base, u32 pin_num)
{
	...
	return !!(readl((unsigned long int)is_addr) & BIT(pin_num));
}
...
static s32 test_gpio_set_input_type(u32 bank_base, u32 pin_num, u32 is)
{
	...
	if (is == 1) {	//Schmitt input
		writel(readl((unsigned long int)is_addr) | BIT(pin_num), (unsigned long int)is_addr);
		/* comment for kernel coding style */
	} else {		//CMOS input
		writel(readl((unsigned long int)is_addr) & (~BIT(pin_num)), (unsigned long int)is_addr);
		/* comment for kernel coding style */
...
static s32 test_gpio_get_slew_rate(u32 bank_base, u32 pin_num)
{
	...
	return !!(readl((unsigned long int)sr_addr) & BIT(pin_num));
}
...
static s32 test_gpio_set_slew_rate(u32 bank_base, u32 pin_num, u32 sr)
{
	...
	if (sr == 1) {	//Slow slew rate
		writel(readl((unsigned long int)sr_addr) | BIT(pin_num), (unsigned long int)sr_addr);
		/* comment for kernel coding style */
	} else {		//Fast slew rate
		writel(readl((unsigned long int)sr_addr) & (~BIT(pin_num)), (unsigned long int)sr_addr);
		/* comment for kernel coding style */
...
static u32 tcc_set_multi_cpu_data(s32 cpu_num)
{
	u32 multi_cpu_data = 0;
	switch (cpu_num) {
		case GPIO_MULTI_CPU0:
			multi_cpu_data = 0x00000000;
			break;
		case GPIO_MULTI_CPU1:
			multi_cpu_data = 0x55555555;
			break;
		case GPIO_MULTI_CPU2:
			multi_cpu_data = 0xAAAAAAAA;
			break;
		case GPIO_MULTI_CPU3:
			multi_cpu_data = 0xFFFFFFFF;
			break;
		default:
			(void)printf("ERROR: Invalid cpu type (%d)", cpu_num);
			break;
	}
	return multi_cpu_data;
}

static void tcc_set_gpio_multi_cpu_control_register(struct udevice *dev, s32 cpu_num, u32 num)
{
	u32 addr;
	struct telechips_pinctrl_priv *priv
		= dev_get_priv(dev);
	addr = priv->gpio_base + GPIO_MULTI_CPU_BASE;
	writel(tcc_set_multi_cpu_data(cpu_num), (unsigned long int)(addr + (num * 4)));
}

static void tcc_set_gpio_multi_cpu(struct udevice *dev, s32 cpu_num)
{
	for (int i = 0; i < GPIO_MULTI_CPU_MAX_NUM; i++) {
		tcc_set_gpio_multi_cpu_control_register(dev, cpu_num, i);
	}
}

static s32 do_test_multi_rw(
		struct cmd_tbl *cmdtp, s32 flag,
		s32 argc, char * const argv[])
{
	struct udevice *dev;
	...
	uclass_foreach_dev_probe(UCLASS_PINCTRL, dev) {
		...
		if (argc == 2) {
			pr_err("argv[%d] : %s\n", i, argv[i]);
			cpu_num = simple_strtoul(argv[1], NULL, 10);
			tcc_set_gpio_multi_cpu(dev, cpu_num);
			for (i = 0; i < ARRAY_SIZE(bank_list); i++) {
				test_rw(dev, bank_list[i].bank_name, -1, cpu_num);
			/* comment for kernel coding style */
			}
		} else {
			pr_err("[FAIL] example : test_gpio multi <cpu_num>(cpu_num : 0~3)\n");
		}
	}
		...
		s32 argc, char * const argv[])
{
	...
	uclass_foreach_dev_probe(UCLASS_PINCTRL, dev) {
		...
		if (argc < 2) {
			...
			if (bank_name != NULL) {
				test_rw(dev, bank_name, pin_num, cpu_num);
			/* comment for kernel coding style */
			}
		}
	}
		...
		s32 argc, char * const argv[])
{
	...
	uclass_foreach_dev_probe(UCLASS_PINCTRL, dev) {
		...
		for (i = 0; i < ARRAY_SIZE(gpio_datasheet_register); i++) {
			read_value
				= readl(
					(unsigned long int)gpio_datasheet_register[i].offset
					+ priv->gpio_base);
		...
		s32 argc, char * const argv[])
{
	...
	pr_err("0x%.8X : 0x%.8X\n", addr, readl((unsigned long int)addr));

	return CMD_RET_SUCCESS;
}


static struct cmd_tbl gpio_subcmd[] = {
	U_BOOT_CMD_MKENT(unit_test, 3, 1, do_gpio_unit_test, "", ""),
	U_BOOT_CMD_MKENT(rw, 3, 1, do_test_rw, "", ""),
	U_BOOT_CMD_MKENT(multi, 3, 1, do_test_multi_rw, "", ""),
...
//	U_BOOT_CMD_MKENT(ex_mux, 4, 1, do_test_eint_port_mux, "", ""),
	U_BOOT_CMD_MKENT(rw_all, 4, 1, do_test_rw_all, "", ""),
};

static s32 do_test_gpio(struct cmd_tbl *cmdtp, s32 flag, s32 argc,
		     char * const argv[])
{
	struct cmd_tbl *cmd;
