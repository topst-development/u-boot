...
static int fdt_do_load(cmd_tbl_t *cmdtp, int flag, int argc,
		       char * const argv[])
{
	unsigned long addr;
	void *hdr_buf;
	...
	addr = simple_strtoul(argv[5], &endp, 16);
	...
	if (strcmp(argv[4], "-")) {
		...
		slot[1] = argv[4][0];
		slot[2] = '\0';
		strncpy(part_name, argv[3], sizeof(part_name) - 2);
		strcat(part_name, slot);
	} else {
		strncpy(part_name, argv[3], sizeof(part_name));
	}

	if (part_get_info_by_dev_and_name_or_num(argv[2], part_name,
						 &dev_desc, &part_info) < 0)
		return CMD_RET_FAILURE;

#if defined(CONFIG_SYS_CACHELINE_SIZE)
	hdr_buf = memalign(CONFIG_SYS_CACHELINE_SIZE, part_info.blksz);
#else
	hdr_buf = malloc(part_info.blksz);
#endif
	if (!hdr_buf)
		return CMD_RET_FAILURE;

	if (blk_dread(dev_desc, part_info.start, 1, hdr_buf) != 1) {
		free(hdr_buf);
		ret = CMD_RET_FAILURE;
	}

	switch (genimg_get_format(hdr_buf)) {
	case IMAGE_FORMAT_FIT:
		size = fdt_totalsize(hdr_buf);
		blkcnt = (size + part_info.blksz - 1) / part_info.blksz;
		buf = map_sysmem(addr, blkcnt * part_info.blksz);
		if (blk_dread(dev_desc, part_info.start, blkcnt, buf) != blkcnt)
			ret = CMD_RET_FAILURE;
		unmap_sysmem(buf);
		break;

	case IMAGE_FORMAT_ANDROID:
		if (android_image_load_dtb(dev_desc, &part_info, addr) != 0)
			ret = CMD_RET_FAILURE;
		break;
...
/* Temporary #ifdef - some archs don't have fdt_blob yet */
		...
		return CMD_RET_SUCCESS;
	} else if (strncmp(argv[1], "lo", 2) == 0) {
		return fdt_do_load(cmdtp, flag, argc, argv);
	}
	 ...
	 */
	} else if ((argv[1][0] == 'p') || (strncmp(argv[1], "li", 2) == 0)) {
	...
	else if (strncmp(argv[1], "optee", 5) == 0) {
		if (!working_fdt)
			return CMD_RET_FAILURE;
		/* Memory should be resized before call OPTEE command */
		if (arch_fixup_fdt(working_fdt)) {
			return CMD_RET_FAILURE;
		}
		if (fdt_parse_optee(working_fdt))
			return CMD_RET_FAILURE;
		return CMD_RET_SUCCESS;
	}
#endif
...
#endif
	"fdt load <interface> <dev[:part|#part_name]> <slot> <addr>\n"
	"                                    - Load a device tree blob from the partition 'part' on\n"
