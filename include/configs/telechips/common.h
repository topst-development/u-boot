...
#define COMMAND_AB \
	"if ab_select slot ${bootdev} ${bootdevnum}#${miscpart}; then " \
		"env set _slot _${slot}; " \
	"else; " \
...
#define CONFIG_FIXED_SDHCI_ALIGNED_BUFFER 0x1F0000000
#else
...
/*
 * Non-Kconfig Configurations
 */

#define CONFIG_SYS_BOOTM_LEN (SZ_64M)
#define CONFIG_SYS_CBSIZE (SZ_2K)
#define CONFIG_SYS_MMC_MAX_BLK_COUNT (1024U)
#define CONFIG_SYS_NONCACHED_MEMORY (SZ_1M) // GMAC DMA descriptor region
