...
#define COMMAND_SUBCORE_BOOT \
	...
	"sc_boot_addr=" __stringify(SUBCORE_ADDR_KERNEL) "\0" \
...
#define COMMAND_SUBCORE_LOADIMG \
	"blkread part ${sc_bootpart}${_slot} ${sc_boot_addr} sc_boot_size; " \
	"blkread part ${sc_rootpart}${_slot} ${sc_root_addr} sc_root_size"

#define COMMAND_SUBCORE_LOADFDT \
	"blkread fdt ${sc_fdtpart}${_slot} ${sc_fdt_addr}"
...
#define COMMAND_SUBCORE_BOOTKERNEL \
	"prepare_subcore ${sc_boot_size} ${sc_fdt_size} ${sc_root_size}; " \
