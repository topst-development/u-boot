...
struct tcc_fb_dm_image_info {
	...
	unsigned int virtual_offset_x;
	unsigned int virtual_offset_y;
...
struct tcc_fb_dm_priv {
	...
	unsigned int lcd_rdma;					/* RDMA number */
	unsigned int lcd_wmix;					/* WMIX number */
	unsigned int lcd_disp;					/* DISP number */
	unsigned int lcd_wdma;					/* WDMA number */
	...
	unsigned int lcd_mux_select;				/* lcd mux select */
	uint32_t ext_lcd_mux_select[PANEL_LCD_MUX_MAX];
	int lcd_mux_select_num;
	unsigned int lcd_mux_bypass;				/* lcd mux bypass */
#if defined(CONFIG_TCC807X)
	unsigned int svsc_trvc_en;				/* trvc enable */
	unsigned int svsc_sdm_en;				/* sdm enable */
#endif
	unsigned long timestamp;		/* Timestamp of display controller activation */
};
