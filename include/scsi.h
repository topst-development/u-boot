...
struct scsi_unmap_block_desc {
	uint64_t	lba;
	uint32_t	cnt;
	uint32_t	reserved;
};

struct scsi_unmap_parameter {
	uint16_t	data_length;
	uint16_t	desc_data_length;
	uint32_t	reserved;
	struct scsi_unmap_block_desc blk_desc;
};
...
struct scsi_ops {
	...
	int (*ufs_query)(const struct udevice *dev, const struct ufs_query_sc *q);
...
/**
 * scsi_ufs_query() - request a ufs query
 *
 * @dev:	SCSI bus
 * @q:		UFS Query Structure
 */
int scsi_ufs_query(struct udevice *dev, struct ufs_query_sc *q);

#if defined(CONFIG_SUPPORT_UFS_RPMB)
/**
 * ufs_rpmb_route_frames() - route RPMB data frames
 * @dev		SCSI bus
...
#else
static inline struct udevice *find_scsi_device(int dev_num)
{ return NULL; }
#endif
#endif
