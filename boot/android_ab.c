...
/*
 * Modified by Telechips Inc. (date: 2023-11)
 */
...
int ab_select_slot(struct blk_desc *dev_desc, struct disk_partition *part_info)
{
	...
	bool inherit_slot = CONFIG_IS_ENABLED(ANDROID_AB_INHERIT);
	char slot_suffix[4];
	...
	if (inherit_slot) {
		if (abc->slot_suffix[0] == '_') {
			slot = BOOT_SLOT_NUM(abc->slot_suffix[1]);
		} else {
