...
#define BOOTCTRL_ACTIVE_PRIORITY	15
#define BOOTCTRL_ACTIVE_TRIES		6
				       ...
				       struct bootloader_control **abc)
{
	...
	*abc = memalign(4096, abc_blocks * part_info->blksz);
#else
		   ...
		   bool store)
{
	...
	slot = -1;
#if !defined(CONFIG_BL3_AB_UPDATE)  /* Remove this code because it is already done before */
	for (i = 0; i < abc->nb_slot; ++i) {
...
#else
    slot = BOOT_SLOT_NUM(abc->slot_suffix[0]);
#endif

	if (store && store_needed) {
		abc->crc32_le = ab_control_compute_crc(abc);
		       ...
		       int slot)
{
	...
	for (i = 0; i < NUM_SLOTS; i++)
		abc->slot_info[i].priority = BOOTCTRL_ACTIVE_PRIORITY - 1;
	abc->slot_info[slot].priority = BOOTCTRL_ACTIVE_PRIORITY;
	abc->slot_info[slot].tries_remaining = BOOTCTRL_ACTIVE_TRIES;
	abc->slot_suffix[0] = BOOT_SLOT_NAME(slot);
	abc->slot_suffix[1] = '\0';
	abc->crc32_le = ab_control_compute_crc(abc);
	ab_control_store(dev_desc, part_info, abc);
