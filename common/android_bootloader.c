...
static int android_bootloader_message_load(struct blk_desc *dev_desc,
...
static int android_bootloader_message_write(struct blk_desc *dev_desc,
					    const disk_partition_t *part_info,
					    struct bootloader_message *message)
{
	ulong message_blocks = sizeof(struct bootloader_message) /
...
android_bootloader_load_and_clear_mode(struct blk_desc *dev_desc,
				       const disk_partition_t *misc_part_info)
{
	struct bootloader_message __aligned(ARCH_DMA_MINALIGN) bcb;
...
char *strjoin(const char **chunks, char separator)
{
...
#endif /* CONFIG_ANDROID_SYSTEM_AS_ROOT */
				 ...
				 unsigned long kernel_address)
{
	...
	char *command_line;
	char slot_suffix[3];
	char *boot_part = ANDROID_PARTITION_BOOT;
	...
	case ANDROID_BOOT_MODE_RECOVERY:
		 ...
		 */
#if !defined(CONFIG_ANDROID_RECOVERY_AS_BOOT)
		boot_part = ANDROID_PARTITION_RECOVERY;
#endif
		break;
	case ANDROID_BOOT_MODE_BOOTLOADER:
...
#if CONFIG_IS_ENABLED(CMD_AVB) || CONFIG_IS_ENABLED(AB_UPDATE)
static bool contains(const char *prop, const char *val)
{
	const char *delim = ",";
	bool found = false;
	char *s;
...
int ft_android_setup(void *blob, bd_t *bd)
{
	...
	parent = fdt_path_offset(blob, "/firmware/android/fstab");
	fdt_for_each_subnode(node, blob, parent) {
		const char *prop;
		char tmp[80];

		prop = fdt_getprop(blob, node, "fsmgr_flags", &len);
		if (!prop)
			continue;
		strncpy(tmp, prop, len);

#if CONFIG_IS_ENABLED(CMD_AVB)
		if (!contains(prop, "avb"))
			strcat(tmp, ",avb");
#endif
#if CONFIG_IS_ENABLED(AB_UPDATE)
		if (!contains(prop, "slotselect"))
			strcat(tmp, ",slotselect");
#endif
		fdt_setprop_string(blob, node, "fsmgr_flags", tmp);
	}
