			...
			unsigned long max_size)
{
	...
	blk_cnt = DIV_ROUND_UP(sizeof(struct andr_img_hdr), part_info->blksz);

	if (blk_dread(dev_desc, part_info->start, blk_cnt, buf) != blk_cnt)
		blk_read = -1;

	if (!blk_read && android_image_check_header(buf) != 0) {
#ifndef CONFIG_AVB_VERIFY
		if (tc_image_check_header((struct tc_img_hdr *)buf) == 0) {
			pr_force("** TC Image header **\n");
		} else
#endif
		{
			pr_err("** Invalid Android Image header **\n");
			blk_read = -1;
		}
	}
	...
	if (!blk_read) {
		...
		if (tc_image_check_header((struct tc_img_hdr *)buf) == 0) {
			...
			tc_h = (struct tc_img_hdr *)buf;

			blk_cnt =
				 (tc_h->img_size + 0x200 + part_info->blksz - 1)
				/ part_info->blksz;
		} else
#endif
		{
			blk_cnt =
				 (android_image_get_end(buf)
				  - (ulong)buf
				  + part_info->blksz - 1)
				/ part_info->blksz;
		}
...
int android_image_load_dtb(struct blk_desc *dev_desc,
			   const disk_partition_t *part_info,
			   unsigned long load_address)
{
	void *hdr_buf;
	long blk_cnt, blk_read;
	int ret = 0;

	blk_cnt = (sizeof(struct andr_img_hdr) + part_info->blksz - 1)
			/ part_info->blksz;
#if defined(CONFIG_SYS_CACHELINE_SIZE)
	hdr_buf = memalign(CONFIG_SYS_CACHELINE_SIZE,
			   blk_cnt * part_info->blksz);
#else
	hdr_buf = malloc(blk_cnt * part_info->blksz);
#endif
	if (!hdr_buf)
		return -1;

	blk_read = blk_dread(dev_desc, part_info->start, blk_cnt, hdr_buf);
	if (blk_read != blk_cnt) {
		ret = -1;
	} else {
		struct andr_img_hdr *hdr = (struct andr_img_hdr *)hdr_buf;
		...
		if (android_image_check_header(hdr) == 0 &&
			...
			dtb_offset += ALIGN(hdr->second_size, hdr->page_size);
			dtb_offset += ALIGN(hdr->recovery_dtbo_size,
					hdr->page_size);
			dtb_offset /= part_info->blksz;
			blk_cnt = ALIGN(hdr->dtb_size, hdr->page_size)
				/ part_info->blksz;

			buf = map_sysmem(load_address, blk_cnt *
					 part_info->blksz);
			blk_read = blk_dread(dev_desc,
					     part_info->start + dtb_offset,
					     blk_cnt, buf);
			if (blk_read != blk_cnt)
				ret = -1;
			unmap_sysmem(buf);
