		       ...
		       int of_size, struct lmb *lmb)
{
	...
	fdt_ret = ft_android_setup(blob, gd->bd);
	if (fdt_ret) {
		printf("ERROR: Android-specific fdt fixup failed: %s\n",
		       fdt_strerror(fdt_ret));
		goto err;
	}
...
#if defined(CONFIG_BOOTSTAGE_FDT)
	ret = fdt_shrink_to_minimum(blob, CONFIG_BOOTSTAGE_FDT_SIZE);
#else
