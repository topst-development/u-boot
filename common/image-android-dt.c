		    ...
		    ulong addr)
{
	...
	hdr = (struct dt_table_header *)buf;
	if (blk_dread(dev_desc, part_info->start, 1, buf) != 1) {
		...
		if (fdt32_to_cpu(hdr->magic) != DT_TABLE_MAGIC) {
			...
			u32 total_size, page_size;
			long blk_cnt, blk_read;

			total_size = fdt32_to_cpu(hdr->total_size);
			page_size = fdt32_to_cpu(hdr->page_size);
			blk_cnt = ALIGN(total_size, page_size)
					/ part_info->blksz;
...
/**
 * Get the index of FDT in memory by its board id and rev in image.
 *
 * @param hdr_addr Start address of DT image
 * @param id ID of desired board
 * @param rev Revision of desired board
 *
 * @return the index FDT on success or -1 on error
 */
int android_dt_get_index(ulong hdr_addr, u32 id, u32 rev)
{
	...
	for (i = 0; i < entry_count; i++) {
		...
		e_id = fdt32_to_cpu(e->id);
		e_rev = fdt32_to_cpu(e->rev);
		unmap_sysmem(e);
		if (e_id == id && e_rev == rev) {
			index = i;
			break;
		}
	}
