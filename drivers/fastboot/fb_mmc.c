...
void fastboot_mmc_erase(const char *cmd, char *response)
{
	...
	lbaint_t pre_start, pre_size, post_start, post_size;
#endif
...
#ifdef CONFIG_ARCH_TELECHIPS
	if (info.start < blks_start) {
		pre_start = info.start;
		pre_size = blks_start - info.start;
		memset((void *)CONFIG_FASTBOOT_BUF_ADDR, mmc->erased_mem_cont,
				(pre_size * info.blksz));
		fb_mmc_blk_write(dev_desc, pre_start, pre_size,
				(const void *)CONFIG_FASTBOOT_BUF_ADDR);
	}

	if ((blks_start + blks_size) < (info.start + info.size)) {
		post_start = blks_start + blks_size;
		post_size = info.start + info.size - post_start;
		memset((void *)CONFIG_FASTBOOT_BUF_ADDR, mmc->erased_mem_cont,
				(post_size * info.blksz));
		fb_mmc_blk_write(dev_desc, post_start, post_size,
				(const void *)CONFIG_FASTBOOT_BUF_ADDR);
	}

	pr_force("Erasing blocks " LBAFU " to " LBAFU " due to alignment\n",
...
#ifdef CONFIG_ARCH_TELECHIPS
	pr_force("........ erased " LBAFU " bytes from '%s'\n",
	       info.size * info.blksz, cmd);
#else
