...
#define TCC_GPIO_OEN	(0x10U)
#define TCC_GPIO_OR		(0x04U)
#define TCC_GPIO_BIC	(0x08U)
#define TCC_GPIO_XOR	(0x0CU)
#define TCC_GPIO_CD0	(0x24U)
#define TCC_GPIO_PE		(0x14U)
#define TCC_GPIO_PS		(0x18U)
#define TCC_GPIO_IEN	(0x1CU)
#define TCC_GPIO_IS		(0x20U)
#define TCC_GPIO_SR		(0x44U)
#define TCC_GPIO_FN0	(0x34U)
#else
#define TCC_GPIO_DATA	(0x00U)
...
#elif defined(CONFIG_TCN100X)
#define TCC_GPIO_COMPATIBLE ("telechips,tcn100x-gpio")
#else
...
static void tcc_pinctrl_dt_bank(struct telechips_pinctrl_priv *priv){
	...
	while (ofnode_valid(node)) {
		...
		if (ofnode_read_u32_array(node, "reg", regs, 4) == 0) {
			priv->bank[i].bank_base = regs[1];
			priv->bank[i].npins = regs[3];
...
static s32 tcc_pinctrl_init_dt(struct udevice *dev,
	...
	node = dev_ofnode(dev);
#if defined(CONFIG_TCN100X)
	if (ofnode_read_u32_array(node, "reg", regs, 8) == 0) {
		//[1] : gpio addr, [3] : gpio range,
		//[5] : pmgpio addr, [7] : pmgpio rnage
		priv->gpio_base = regs[1];
		priv->pmgpio_base = regs[5];
#else
	if (ofnode_read_u32_array(node, "reg", regs, 4) == 0) {
	...
	(u32 bank_base, u32 pin_num, u32 ds, const struct udevice *dev)
{
	...
	pctl_add_wrap(ds_addr, ((pin_num / 8U) * 0x4U), &ds_addr);
	u32 bit_shift = (pin_num % 8U) * 4U;
	...
	(u32 bank_base, u32 pin_num, u32 sr, const struct udevice *dev)
{
	...
	pctl_add_wrap(sr_addr, ((pin_num / 8U) * 0x4U), &sr_addr);
	u32 bit_shift = (pin_num % 8U) * 4U;

	return integrated_gpio_writel
			(sr_addr, bit_shift, 1U, sr, dev);
#elif defined(CONFIG_TCC807X)
	return 0;
#else
