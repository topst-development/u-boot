...
static void check_params(struct phy_configure_opts_pcie *config)
{
	if (config != NULL) {
		...
		if (config->tx_clk_swing_lvl > ERIO_TX_LVL_CTRL_MAX) {
			config->tx_clk_swing_lvl = ERIO_TX_LVL_CTRL_MAX;
		}
	}
}
...
static int32_t sf_pcie_phy_set_clk_swing_lvl(const struct sf_pcie_phy *priv,
		const struct phy_configure_opts_pcie *config)
{
	int32_t err = 0;
	...
	if ((priv != NULL) && (config != NULL)) {
		...
		val = ((u32)config->tx_clk_swing_lvl << ERIO_TX_LVL_CTRL_SHIFT);
		sf_pcie_phy_writel(priv->clk_base, PCIE_CLK_CFG04, val, mask);
	} else {
		err = -EINVAL;
	}
...
static int32_t sf_pcie_phy_configure(struct phy *phy, void *params)
{
	...
	if ((phy != NULL) && (params != NULL)) {
		...
		if (err == 0) {
			err = sf_pcie_phy_set_clk_swing_lvl((const struct sf_pcie_phy *)priv,
					(const struct phy_configure_opts_pcie *)config);
		}
	} else {
		err = -EINVAL;
...
static s32 sf_pcie_phy_revert_to_default(const struct sf_pcie_phy *priv)
{
	int32_t err = 0;
	...
	if (priv != NULL) {
		...
		val = 0x0U;
		sf_pcie_phy_writel(priv->pma_base, PCIE_PMA_CMN_REG17B, val, mask);

		mask = PCIE_PMA_LN0_OV_S_ANA_TX_DRV_IDRV_EN_MASK;
		val = mask;
		sf_pcie_phy_writel(priv->pma_base, PCIE_PMA_TRSV_REG40B, val, mask);

		mask = PCIE_PMA_LN0_OV_S_PI_OFFSET_QCLK_MASK;
		val = 0x0U;
		sf_pcie_phy_writel(priv->pma_base, PCIE_PMA_TRSV_REG5FD, val, mask);
	} else {
		err = -EINVAL;
	}
...
static int32_t sf_pcie_phy_init(struct phy *phy)
{
	...
	if (phy != NULL) {
		...
		err = sf_pcie_phy_revert_to_default(priv);
		if (err == 0) {
			uint32_t val, mask, idx;
			...
			val = 0x700D5U;
			sf_pcie_phy_writel(priv->pcs_base, PCIE_PCS_OUT_VEC_4, val, mask);
		}
	} else {
		err = -ENODEV;
	}
...
static int32_t sf_pcie_phy_probe(struct udevice *dev)
{
	...
	if (dev != NULL) {
		...
		if (err == 0) {
			priv->pcs_base = dev_remap_addr_index(dev, 2);
			if (priv->pcs_base == NULL) {
				err = -EINVAL;
			}
		}

		if (err == 0) {
			priv->clk_base = dev_remap_addr_index(dev, 3);
