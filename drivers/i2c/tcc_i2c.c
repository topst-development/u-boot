...
static uint8_t tcc_i2c_get_pcfg(const struct tcc_i2c *i2c, int32_t ch)
{
	...
	uint8_t ret = 0xFFU;
	int32_t last_cont_idx = i2c->soc_info->last_ch + i2c->soc_info->slv_cnt;

	if ((ch >= 0) && (ch <= last_cont_idx)) {
		if (i2c->soc_info->id != TCN100X) {
			if (ch < 4) {
				offset = I2C_PCFG0;
			} else if (ch < 8) {
				offset = I2C_PCFG2;
		...
		port = readl(base + offset);
		port >>= shift;
		ret = (u8)(port & (u32)0x3FU);
	}
...
{
	uint32_t temp_u32, shift, offset;
	void __iomem *base = i2c->pcfg;
	int32_t last_cont_idx = i2c->soc_info->last_ch + i2c->soc_info->slv_cnt;
	...
	if ((ch >= 0) && (ch <= last_cont_idx)) {
		...
		if (shift <= (3U * (u8)BITS_PER_BYTE)) {
			temp_u32 = readl(base + offset);
			temp_u32 &= ~((u32)0x3FU << shift);
			temp_u32 |= ((u32)port << shift);
			writel(temp_u32, base + offset);
		}
	}
}
...
static int32_t tcc_i2c_set_port(const struct tcc_i2c *i2c)
{
	...
	uint8_t port = (u8)(i2c->port & 0x3FU);
	uint8_t conflict;
	int32_t i, last_cont_idx, ret = 0;
	...
	if (ch < 0) {
		...
		last_cont_idx = i2c->soc_info->last_ch + i2c->soc_info->slv_cnt;
		for (i = 0; i <= last_cont_idx; i++) {
			if (i == ch) {
				continue;
			}
			conflict = tcc_i2c_get_pcfg(i2c, i);
			if (port == conflict) {
				tcc_i2c_set_pcfg(i2c, i, (u8)0x3FU);
			}
		}
	}
...
static const struct udevice_id tcc_i2c_ids[] = {
	...
	{	.compatible = "telechips,tcn100x-i2c",
		.data = (ulong)&tcn100x_soc_info},
	{ }
};
