...
int32_t pcie_dw_tcc_set_phy_conf(void *params)
{
	...
	if ((g_pDevPriv != NULL) && (params != NULL)) {
		struct pcie_dw_tcc *priv = g_pDevPriv;
		struct pcie_dw *pci = &priv->dw;
		struct udevice *dev = pci->dev;

		err = pcie_dw_tcc_phy_setup(dev, &priv->phy);
		if (err == 0) {
			/* assert reset gpio */
			err = pcie_dw_tcc_set_reset_gpio(dev, 0);
		}
		...
		if (err == 0) {
			/* clear all interrupt signals */
			err = pcie_dw_tcc_clear_interrupt(priv);
		}

		if (err == 0) {
			/* assert global reset */
			err = pcie_dw_tcc_assert_global_reset(priv);
		}

		if (err == 0) {
			err = generic_phy_reset(&priv->phy);
		}

		if (err == 0) {
			err = pcie_dw_tcc_set_slv_addr_mask(
					(const struct pcie_dw_tcc *)priv);
		}

		if (err == 0) {
			/* set PCIe controller mode rc or ep*/
			err = pcie_dw_tcc_set_mode(
					(const struct pcie_dw_tcc *)priv, PCIE_DEVICE_RC);
		}

		if (err == 0) {
			/* set reference clock source */
			err = pcie_dw_tcc_set_refclk(priv);
		}

		if (err == 0) {
			err = generic_phy_configure(&priv->phy, params);
		}

		if (err == 0) {
			err = pcie_dw_tcc_gen4_phy_setup(priv);
		}

		if (err == 0) {
			/* set PCIe Target Link Speed */
			err = pcie_dw_tcc_set_max_link_speed(priv);
		}

		if (err == 0) {
			err = pcie_dw_tcc_set_layer(priv);
		}
		...
		if (err == 0) {
			(void)pcie_dw_tcc_establish_link(priv);
		}
	} else {
		err = -EINVAL;
	}
