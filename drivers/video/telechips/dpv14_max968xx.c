...
#define SERDES_WATCH_LOCK_PIN           0xDFFF
...
static struct Max968xx_Reg_Data pstVIC_1027_Max968xx_ES3_Reg[] = {
	...
	*/
	...
	{SERDES_WATCH_LOCK_PIN, DP_SER_DES_INVALID_REG_ADDR,
...
static void max968xx_set_write_data(bool blane_reswap,
					...
					const struct Max968xx_Reg_Data *pstmax968xx_reg)
{
	uint8_t uclo_lane0, uclo_lane1, uclo_lane2, uclo_lane3;
	uint8_t ucreg_evb_t;
	uint32_t uireg_add;
	const struct MAX968XX_Drv_Params *pstmax968xx_params;
	...
	if ((ucnum_of_dps == 1U) && (uireg_add == (uint32_t)SER_MISC_CONFIG_B1)) {
		debug_pr(LOGL_DEBUG, "[%s:%d]Set to SST...", __func__, __LINE__);

		*pucw_buf = (uint8_t)MST_FUNCTION_DISABLE;
	}

	if ((ucnum_of_dps    > 1U) && (uireg_add == (uint8_t)DES_STREAM_SELECT)) {
		if (pstmax968xx_params->aucvcp_id[ucdp_id] > 0U) {
			*pucw_buf = (uint8_t)((pstmax968xx_params->aucvcp_id[ucdp_id] - 1U) & 0xFFU);
		}

		debug_pr(LOGL_INFO, "[%s:%d]DP%d -> Set VCP id to %u\n", __func__, __LINE__, ucdp_id, *pucw_buf);
	}

	if ((ucnum_of_dps == 1U) && (ucreg_evb_t == (uint8_t)TCC_EVB_LCD_ONE_POW) &&  (uireg_add == 0x0079U)) {
		debug_pr(LOGL_INFO, "[%s:%d] Serializer Enable only I2C PT 1(1Des)\n", __func__, __LINE__);
	...
	if ((ucnum_of_dps < 3U) && (ucreg_evb_t == (uint8_t)TCC_EVB_LCD_FOUR_POW) && (uireg_add == 0x0079U)) {
		debug_pr(LOGL_INFO, "[%s:%d]Serializer enables I2C PT1(1 Des), PT2(1Des)\n", __func__, __LINE__);
											...
											const struct Max968xx_Reg_Data *pstmax968xx_reg)
{
	...
	if (!bcontinue) {
		if ((ucnum_of_dps == 1U) &&
			((uireg_add == (uint32_t)DES_DROP_VIDEO) ||
			(uireg_add == (uint32_t)DES_STREAM_SELECT))) {
			bcontinue = (bool)true;
		}
	}
	if (!bcontinue) {
...
static int max968xx_des_is_locked(uint8_t ucNumOfDPs, uint32_t timeout)
{
	const struct MAX968XX_Drv_Params *pstmax968xx_params;
	uint8_t read_data = 0u, ucdp_idx;
	int res_i2c;
	uint8_t mask = 0xFFu;
	uint8_t status = 0u;
	ulong start_time = get_timer(0), end_time = 0u;

	if (ucNumOfDPs <= 4u) {
		mask = (uint8_t)(((1u << ucNumOfDPs) - 1u) & 0xFFu);
	}

	for (ucdp_idx = 0; ucdp_idx < ucNumOfDPs; ucdp_idx++) {
		pstmax968xx_params = (const struct MAX968XX_Drv_Params *)&stmax968xx_params[ucdp_idx];

		if (pstmax968xx_params->pstdes_i2c_udevice == NULL) {
			debug_pr(LOGL_INFO, "[%s:%d]Des(0x%02x) is not connected\n",
			__func__, __LINE__, pstmax968xx_params->ides_i2c_devadd);
			status |= 1u << ucdp_idx;
		}
	}
	...
	while (status != mask) {
		for (ucdp_idx = 0; ucdp_idx < ucNumOfDPs; ucdp_idx++) {
			if (((status >> ucdp_idx) & 1u) == 1u) {
				continue;
			}

			pstmax968xx_params = &stmax968xx_params[ucdp_idx];
			res_i2c = dm_i2c_read(pstmax968xx_params->pstdes_i2c_udevice,
					  (uint)DES_CTRL3, &read_data, 1);

			if ((res_i2c == 0) && ((read_data & 0x08u) == 0x08u)) {
				status |= 1u << ucdp_idx;
			}
		}
		mdelay(1);
...
static int32_t max968xx_set_reg_tables(uint8_t ucNumOfDPs, uint8_t ucTableIdx)
{
	...
	for (uclane_idx = 0; uclane_idx < (uint8_t)DP_LANE_4; uclane_idx++) {
	...
	if (ret == 0) {
		...
		for (uielements = 0;
			...
			if (pstmax968xx_reg[uielements].ii2c_dev_add == (int32_t)SERDES_DELAY_ADDR) {
				...
				continue;
			} else if (pstmax968xx_reg[uielements].ii2c_dev_add == (int32_t)SERDES_WATCH_LOCK_PIN) {
				ret = max968xx_des_is_locked(ucNumOfDPs, pstmax968xx_reg[uielements].uireg_val);
				if (ret != 0) {
					debug_pr(LOGL_ERR, "[%s:%d]Error: Des Lock pin is not locked", __func__, __LINE__);
				}
				continue;
			} else {
...
static int max968xx_get_reg_table_idx(uint32_t uivic, uint8_t *puctable_idx)
{
	...
	uint32_t ucelements, ucnum_of_t;
	int32_t ret = 0;
	uint32_t uicus_vic;

	ucnum_of_t = (uint32_t)(((sizeof(stmax968xx_reg_params) / sizeof(struct Max968xx_Reg_Params))) & 0xFFFFFFFFU);
	...
	if (ret == 0) {
		...
		if (bfound_vic) {
			debug_pr(LOGL_DEBUG, "[%s:%d]found vic from table %u\n", __func__, __LINE__, ucelements);
			*puctable_idx = (uint8_t)(ucelements & 0xFFU);
		} else {
...
static int32_t max968xx_get_des_rev(uint8_t ucdp_index, uint8_t uci2c_port)
{
	...
	if (ret == 0) {
		for (uctry = 0; uctry < (uint8_t)MAX_I2C_CHECK_RETRY; uctry++) {
			...
			if (ret == 0) {
				debug_pr(LOGL_DEBUG, "[%s:%d]Get Des uclass handle from add 0x%x\n", __func__, __LINE__, pstmax968xx_params->ides_i2c_devadd);
				break;
			}
		}
	...
	if (ret == 0) {
		for (uctry = 0; uctry < (uint8_t)MAX_I2C_CHECK_RETRY; uctry++) {
			ret = dm_i2c_read(pstmax968xx_params->pstdes_i2c_udevice,
					  (uint)DES_DEV_REV, &ucdes_rev, 1);
			if (ret == 0) {
				pstmax968xx_params->ucdes_rev = ucdes_rev;

				debug_pr(LOGL_INFO, "[%s:%d]Get Des revision as %u\n",
...
static int32_t max968xx_get_ser_rev(uint8_t ucdp_index, uint8_t uci2c_port)
{
	...
	if (ret == 0) {
		if (iser_devadd > 0) {
			...
			debug_pr(LOGL_ERR, "\n[%s:%d]Err: invalid Ser dev add as 0x%x\n",
				 __func__, __LINE__, pstmax968xx_params->iser_i2c_devadd);
	...
	if (ret == 0) {
		for (uctry = 0; uctry < (uint8_t)MAX_I2C_CHECK_RETRY; uctry++) {
			...
			if (ret == 0) {
				debug_pr(LOGL_DEBUG, "[%s:%d]Get Ser uclass handle from add 0x%x\n",
	...
	if (ret == 0) {
		for (uctry = 0; uctry < (uint8_t)MAX_I2C_CHECK_RETRY; uctry++) {
			ret = dm_i2c_read(pstmax968xx_params->pstser_i2c_udevice,
					  (uint)SER_DEV_REV, &ucser_rev, 1);
			if (ret == 0) {
				pstmax968xx_params->ucser_rev = ucser_rev;

				debug_pr(LOGL_INFO, "[%s:%d]Get Ser rev as %u\n", __func__, __LINE__, ucser_rev);
				break;
			}
		}
	}
...
int32_t max968xx_init(const struct dpv14_drv_params *drv_params)
{
	...
	int32_t ret = 0;
	struct MAX968XX_Drv_Params *pstmax968xx_params;
	const struct MAX968XX_Drv_Params *pstmax968xx_list;

	pstmax968xx_params = &stmax968xx_params[MAX968XX_INPUT_STREAM_0];

	pstmax968xx_params->ucevb_power_type = drv_params->evb_power_type;

	(void)memcpy(&pstmax968xx_params->aucvcp_id[0],
		    &drv_params->vcp_id[0],
		    sizeof(uint8_t) * (unsigned)MAX968XX_INPUT_STREAM_MAX);
	(void)memcpy(&pstmax968xx_params->auclane_order[0],
		     &drv_params->lane_order[0],
		     sizeof(uint8_t) * (unsigned)DP_LANE_4);

	(void)max968xx_set_i2c_speed(drv_params->i2c_port);

	(void)max968xx_get_ser_rev((uint8_t)MAX968XX_INPUT_STREAM_0,
				   drv_params->i2c_port);

	for (ucdp_idx = 0u; ucdp_idx < drv_params->num_of_dps; ucdp_idx++) {
		(void)max968xx_get_des_rev(ucdp_idx, drv_params->i2c_port);
	...
	ret = max968xx_get_reg_table_idx(drv_params->vic[MAX968XX_INPUT_STREAM_0][VIC_CFG_VIDEO_CODE],
				  &uctable_idx);
	if (ret == 0) {
		/* For KCS */
		(void)max968xx_set_reg_tables(drv_params->num_of_dps, uctable_idx);
	}

	debug_pr(LOGL_INFO, "\n[%s:%d]Init -> Max968xx\n", __func__, __LINE__);
	debug_pr(LOGL_INFO, " I2C Port : %u\n", drv_params->i2c_port);
	debug_pr(LOGL_INFO, " Ser I2C add: 0x%x -> Des I2C add: 0x%x\n",
				pstmax968xx_params->iser_i2c_devadd,
				pstmax968xx_params->ides_i2c_devadd);
	debug_pr(LOGL_INFO, " Ser Rev. %u, Des Rev. %u\n",
				pstmax968xx_params->ucser_rev,
				pstmax968xx_params->ucdes_rev);
	debug_pr(LOGL_INFO, " Evb : %s\n",
	...
	debug_pr(LOGL_INFO, " VIC : %u\n", drv_params->vic[MAX968XX_INPUT_STREAM_0][VIC_CFG_VIDEO_CODE]);
