...
static int max968xx_des_is_locked(uint8_t ucNumOfDPs, uint32_t timeout)
{
	...
	uint8_t read_data = 0, ucdp_idx;
	int res_i2c;
	uint8_t mask = (1 << ucNumOfDPs) - 1, status = 0;
	ulong start_time = get_timer(0), end_time = 0;

	while (status != mask) {
		for (ucdp_idx = 0; ucdp_idx < ucNumOfDPs; ucdp_idx++) {
			if (((status >> ucdp_idx) & 1) == 1) {
				continue;
			}
			...
			if ((res_i2c == 0) && ((read_data & 0x08) == 0x08)) {
				status |= 1 << ucdp_idx;
			}
		}
		mdelay(1);
...
int32_t max968xx_init(const struct MAX968XX_Init_Params *pstinit_params)
{
	...
	for (ucdp_idx = 0u; ucdp_idx < pstinit_params->ucnum_of_dps; ucdp_idx++) {
