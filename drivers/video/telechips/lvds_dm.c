		   ...
		   const struct display_timing *timings)
{
	...
	(void)lcdc_mux_select(priv->lvds_info.lcdc_mux_id, get_vioc_index(fb_priv->lcd_disp));
		...
		while(ret == 0) {
			...
			if (!ofnode_valid(remote)) {
				...
				if (fb_dev != NULL) {
					...
					pr_info("[INFO][%s](%s): lcd_mux_id = %u\n", __func__,
						dev_read_name(dev), priv->lvds_info.lcdc_mux_id);
					break;
				}
			}
		}
...
static int tcc_lvds_ofdata_to_platdata(struct udevice *dev)
{
	...
	if (ofnode_device_is_compatible(dev_ofnode(dev), "telechips,fb-auoc123han06")) {
	...
	*/
	...
	if (dt_lvds_type == (u32)LVDS_DUAL) {
		...
		num_val = 2;
	} else if (dt_lvds_type == (u32)LVDS_SINGLE) {
	...
	ret = ofnode_read_u32(dev_ofnode(dev), "lcdc-mux-bypass", &priv->lvds_info.lcdc_mux_bypass);
	if (ret < 0) {
