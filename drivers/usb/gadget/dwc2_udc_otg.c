...
static void reconfig_usbd(struct dwc2_udc *dev)
{
	...
	gsnpsid = readl(&reg->res1[4]) & 0xffff;
	uTemp = writel(CORE_SOFT_RESET, &reg->grstctl);
	 ...
	 */
	if (gsnpsid >= 0x420a) {
		while ((readl(&reg->grstctl) & CORE_SOFT_RESET_DONE) == 0) {
			debug("%s: waiting for CORE_SOFT_RESET_DONE\n", __func__);
			udelay(10);
		}
...
EXPORT_SYMBOL_GPL(dwc2_udc_handle_interrupt);
...
#if !defined(CONFIG_TELECHIPS_DWC2_PHY) || !defined(CONFIG_USB_DWC3_GENERIC)
int dm_usb_gadget_handle_interrupts(struct udevice *dev)
{
	return dwc2_udc_handle_interrupt();
}
#endif /* !defined(CONFIG_TELECHIPS_DWC2_PHY) */
...
#if !defined(CONFIG_TELECHIPS_DWC2_PHY)
...
static int dwc2_disable_vbus_supply(struct udevice *dev)
{
	struct dwc2_priv_data *priv = dev_get_priv(dev);
	int ret = 0;
	const char *str;

	str = ofnode_read_string(dev_ofnode(dev), "pinctrl-names");
	if (str == NULL) {
		dev_info(dev, "There is no pinctrl names\n");
		return ret;
	}
...
static int dwc2_udc_otg_probe(struct udevice *dev)
{
	...
	ret = dwc2_disable_vbus_supply(dev);
	if (ret) {
		return ret;
	}
