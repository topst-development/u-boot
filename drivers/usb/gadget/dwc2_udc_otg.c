...
void reconfig_usbd(struct dwc2_udc *dev);
...
#ifdef DEBUG
	pr_debug("calling complete callback\n");
	{
...
static int dwc2_enable_vbus_supply(struct udevice *dev)
{
	int ret;
...
int dm_usb_20_gadget_handle_interrupts(struct udevice *dev)
{
	return dwc2_udc_handle_interrupt();
}
...
static int dwc2_udc_otg_probe(struct udevice *dev)
{
	...
	ret = dwc2_enable_vbus_supply(dev);
	if (ret)
		return ret;

	dwc2_disable_vbus_supply(dev);
#if 0
