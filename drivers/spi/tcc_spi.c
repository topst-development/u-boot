...
static int32_t tcc_spi_set_port(const struct udevice *bus)
{
	...
	if (ch < 0) {
		...
		/* check port conflict */
		for (i = 0; i <= tccspi->soc_info->last_ch; i++) {
			if (tccspi->soc_info->id != TCN100X) {
				if (i < 4) {
				...
				pcfg_offset = (u32)i * 4U;
				offset = 0U;
			}

			pcfg_val = readl(tccspi->pcfg + pcfg_offset);
			if (offset <= 24U) {
...
static const struct tcc_spi_soc_info tcn100x_spi_info = {
	.id = TCN100X,
	.last_ch = 5,
};
