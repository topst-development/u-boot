	...
	uint32_t p_port, uint32_t s_port, uint32_t freq)
{
	...
	if ((bool)p_reg) {
		...
		if (found_ratio == 0u) {
			debug_pr(LOGL_ERR, "[ERROR][%s] : can not get upsample ratio (%dMhz)\n",
				__func__, (pxclk / 1000000u));
		}
	} else {
		debug_pr(LOGL_ERR, "[ERROR][%s] : can not get hw address\n", __func__);
	}
	...
	uint32_t upsample_ratio)
{
	...
	if ((bool)p_reg) {
		...
		if (upsample_ratio > LVDS_PHY_UPSAMPLE_RATIO_MAX) {
			debug_pr(LOGL_ERR, "[ERROR][%s] : invalid parameter (pxclk:%dMhz / upsample_ratio:%d)\n",
	...
	uint32_t pn_swap)
{
	...
	if ((bool)phy_reg) {
		...
		default:
			debug_pr(LOGL_ERR, "[ERROR][%s] : invalid parameter(lane:%u)\n", __func__, lane);
			break;
		}
	}
}
...
/* LVDS_PHY_SetFcon
 * Setup LVDS PHY Manual/Automatic Coarse tuning
 * port : the port number of lvds phy
 * mode : coarse tuning method 0-manual, 1-automatic
	...
	uint32_t division, uint32_t fcon)
{
	...
	if ((bool)phy_reg) {
		...
		case LVDS_PHY_FCON_MANUAL:
			debug_pr(LOGL_INFO, "[INFO][%s] : manual tuning, port = %u\n", __func__, port);
		...
		case LVDS_PHY_FCON_AUTOMATIC:
			debug_pr(LOGL_INFO, "[INFO][%s] : automatic tuning, port = %u\n", __func__, port);
		...
		default:
			debug_pr(LOGL_ERR, "[ERROR][%s] : invalid parameter(mode: %u)\n", __func__, mode);
			break;
		}
	}
}
...
void LVDS_PHY_SetCFcon(uint32_t port, uint32_t mode, uint32_t enable)
{
	...
	if ((bool)phy_reg) {
		...
		if (time_out >= 100u) {
			debug_pr(LOGL_INFO, "[INFO][%s] : timeout(pd_fcstat: 0x%08X)\n", __func__, pd_fcstat);
		}
...
void LVDS_PHY_CheckPLLStatus(uint32_t p_port, uint32_t s_port)
{
	...
	if ((bool)p_reg) {
		...
		if (time_out >= 100u) {
			debug_pr(LOGL_INFO, "[INFO][%s] : PLL locking timeout\n", __func__);
		}
	}
}
	...
	uint32_t sfcon)
{
	...
	debug_pr(LOGL_INFO, "[INFO][%s] :[%s] PLL locking loop_cnt = %u, lock_cnt = %u\n", __func__,
	...
	uint32_t phy_reg, uint32_t offset, uint32_t value)
{
	...
	if ((phy_offset < UINT_MAX) && (strobe_done_offset < UINT_MAX)) {
		...
		if (time_out >= 10u) {
			debug_pr(LOGL_INFO, "[INFO][%s] : timeout\n", __func__);
		}
	}
}
	...
	uint32_t step, uint32_t vcm, uint32_t vsw)
{
	...
	if ((bool)p_reg) {
		debug_pr(LOGL_INFO, "[INFO][%s] : step %u\n", __func__, step);
		switch (step) {
		case LVDS_PHY_INIT:
			...
			#if defined(DUAL_CLK_MODE_MAIN)
				LVDS_PHY_StrobeWrite(s_reg, 0x3B8u, 0x00000003u);
			#elif defined(DUAL_CLK_MODE_SUB)
				LVDS_PHY_StrobeWrite(p_reg, 0x3B8u, 0x00000003u);
			#endif
			}
...
//		case LVDS_PHY_CONFIG_MAX:
		default:
			debug_pr(LOGL_ERR, "[ERROR][%s] : invalid parameter(step: %u)\n", __func__, step);
			break;
		}
	}
}
