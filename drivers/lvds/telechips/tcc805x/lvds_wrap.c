...
void LVDS_WRAP_SetDataSwap(uint32_t ch, uint32_t set)
{
	...
	case 3:
		...
		break;
	default:
		pr_err("%s: invalid parameter(%u, %u)\n", __func__, ch, set);
		break;
	}
}
	...
	uint32_t select, uint32_t enable)
{
	...
	if((bool)err_mux_output) {
		pr_err("%s in error, invalid parameter(mux: %u, ch: %u)\n",__func__,
			mux, ch);
	...
	uint32_t ch, uint32_t path, uint32_t set)
{
	...
	if(!(bool)offset) {
		pr_err("%s in error, invalid parameter(ch: %u, path: %u)\n", __func__,ch, path);
	}
}
...
void LVDS_WRAP_SM_Bypass(uint32_t lcdc_mux_id, uint32_t en)
{
	uint32_t val;
	uint32_t bypass_ = ((bool)en) ? 1u : 0u;

	switch (lcdc_mux_id) {
	case 2u:
		val = (LVDS_wrap_readl(REG_LVDS_WRAP + SM_BYPASS) &
			~(0x1UL << SHIFT_SDC));
		val |= (bypass_ << SHIFT_SDC);
		LVDS_wrap_writel(val, REG_LVDS_WRAP + SM_BYPASS);
		break;
	case 3u:
		val = (LVDS_wrap_readl(REG_LVDS_WRAP + SM_BYPASS) &
			~(0x1UL << SHIFT_SRC));
		val |= (bypass_ << SHIFT_SRC);
		LVDS_wrap_writel(val, REG_LVDS_WRAP + SM_BYPASS);
		break;
	default:
		/*
	...
	pr_info("%s : lcdc mux id = %u, SM_BYPASS = 0x%08x\n", __func__,
		lcdc_mux_id, LVDS_wrap_readl(REG_LVDS_WRAP + SM_BYPASS));
}
/* end of file */
