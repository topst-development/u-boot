...
#if defined(CONFIG_TCN100X)
		shift = 0;
		tmp_portcfg = portcfg_base + (num*4U);
#else
		shift = (num % 4U) * 8U;
...
{
#if (!defined(CONFIG_TCC750X) && !defined(CONFIG_TCN100X))
...
#if defined(CONFIG_TCC807X) || defined(CONFIG_TCC805X) || defined(CONFIG_TCN100X)
