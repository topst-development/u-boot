...
static void tcc_sdhci_clk_set(unsigned long id, unsigned long en,
		unsigned long rate, unsigned long flags)
{
#if defined(CONFIG_TCN100X)
	(void)tcc_set_clkctrl(id, en, rate, flags);
#else
	(void)tcc_set_peri(id, en, rate, flags);
#endif
}

static unsigned long tcc_sdhci_clk_get(unsigned long id)
{
	unsigned long ret = 0;
#if defined(CONFIG_TCN100X)
	ret = tcc_get_clkctrl(id);
#else
	ret = tcc_get_peri(id);
#endif
	return ret;
...
static int tcc_sdhci_clk_init(struct udevice *dev)
{
	...
	if (enable_clk_conf != 0) {
		if (use_iobus == true ) {
			...
			(void)tcc_sdhci_clk_set(peri_id, CKC_ENABLE, plat->peri_clk, 0);
		} else { /* tcc750x, tcn100x */
			//(void)tcc_ckc_swreset(pswrst_id, CKC_DISABLE);

			(void)tcc_sdhci_clk_set(peri_id, DISABLE, plat->peri_clk, 0);
			(void)tcc_sdhci_clk_set(peri_id, CKC_ENABLE, plat->peri_clk, 0);
	...
	if (use_high_peri != 0) {
		if (enable_clk_conf != 0) {
			...
			get_peri = tcc_sdhci_clk_get(peri_id);
			(void)tcc_sdhci_clk_set(peri_id, DISABLE, plat->peri_clk, 0);
		} else {
	...
	if (enable_clk_conf != 0) {
		pr_debug("[DEBUG] %s: actual peri clock: %lu\n",
				dev->name, tcc_sdhci_clk_get(peri_id));
	}
...
static int tcc_sdhci_probe(struct udevice *dev)
{
	...
	if ((plat->cfg.host_caps & MMC_CAP_NONREMOVABLE) != 0u) {
		/* Set CAP register */
#if defined(CONFIG_TCN100X)
		//Set CAP0_0[25] bit(64-bit System) for ADMA 64-bit
		writel(0x0FFFE470, plat->chctrl_base + SDHC_CHCTRL_CAP_0);
#else
...
static const struct udevice_id tcc_sdhci_ids[6] = {
	...
	{ .compatible = "telechips,tcn100x-sdhci" },
	{ }
};
