...
static int tcc_sc_mmc_wait_dat0(struct udevice *dev, int state, int timeout_us)
{
	/*
	 * If the driver doesn't support wait_dat0(), __mmc_switch() calls
	 * mdelay(timeout_ms).
	 * To reduce the delay, this function always returns 0.
	 */
...
#endif
	.wait_dat0	= tcc_sc_mmc_wait_dat0,
#if CONFIG_IS_ENABLED(MMC_HS400_ES_SUPPORT)
