...
static int mmc_select_mode(struct mmc *mmc, enum bus_mode mode)
{
	...
	pr_err("selecting mode %s (freq : %d MHz)\n", mmc_mode_name(mode),
...
static int mmc_read_blocks(struct mmc *mmc, void *dst, lbaint_t start,
			   lbaint_t blkcnt)
{
#if CONFIG_IS_ENABLED(MMC_TCC_CLOSE_ENDED)
	struct mmc_cmd sbc;
#endif
	struct mmc_cmd cmd;
	struct mmc_data data;
	...
	if (blkcnt > 1) {
		...
		sbc.cmdarg = blkcnt & 0x0000FFFF;
		sbc.resp_type = MMC_RSP_R1;
		if (mmc_send_cmd(mmc, &sbc, NULL))
			return 0;
#endif
	} else {
		      ...
		      enum mmc_hwpart_conf_mode mode)
{
	...
	if (!(ext_csd[EXT_CSD_ERASE_GROUP_DEF] & 0x01)) {
		...
		mmc->erased_mem_cont = ext_csd[181];
	}
	...
	for_each_mmc_mode_by_pref(card_caps, mwt) {
		for_each_supported_width(card_caps & mwt->widths,
			...
			if (!err)
				return 0;
error:
#if CONFIG_IS_ENABLED(MMC_TCC_NO_RETRY_INIT)
			pr_err("unable to select a mode\n");
			return -ENOTSUPP;
#endif
