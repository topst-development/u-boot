...
#define CLK_TFA_VERSION(major, minor, patch) ({			\
	unsigned long version;					\
								\
	version  = ((major) <= 999UL) ? ((major)*1000000UL) : 0UL;	\
	version += ((minor) <= 999UL) ? ((minor)*1000UL) : 0UL;	\
	version += ((patch) <= 999UL) ? (patch) : 0UL;		\
	version;						\
})

#define CLK_TFA_VERSION_RAW(major, minor, patch)   \
	((((major)*1000000UL) + ((minor)*1000UL) + (patch)))

#define CHECK_TFA_VERSION_MIN(major, minor, patch) \
	(CLK_TFA_VERSION_RAW((major), (minor), (patch)) <= dm_vtfa_clk)

#define CHECK_TFA_VERSION_IS(major, minor, patch) \
	(CLK_TFA_VERSION_RAW((major), (minor), (patch)) == dm_vtfa_clk)

static unsigned long dm_vtfa_clk;
...
static int tcc_ckc_fbus_probe(struct udevice *dev)
{
	...
	dm_vtfa_clk = CLK_TFA_VERSION(res.a0, res.a1, res.a2);
					   ...
					   unsigned long rate)
{
	...
	if (CHECK_TFA_VERSION_IS(1UL, 1UL, 0UL)) {
		ret = res.a3;
	} else {
		ret = res.a0;
	}
