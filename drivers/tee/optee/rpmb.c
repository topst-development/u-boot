...
#define UFS_DEV_ID		(0x3)
#define RPMB_CID_SIZE		(16)
...
static u32 rpmb_get_dev_info(u16 dev_id, struct rpmb_dev_info *info)
{
	...
	struct udevice *ufs = find_scsi_device(UFS_DEV_ID);
	int i;
	...
	if (mmc) {
		...
		const struct ufs_desc_info *ufs_desc = ufs_get_desc_info(ufs);

		if (ufs_desc == NULL) {
			return TEE_ERROR_GENERIC;
		}

		(void)memcpy(info->cid, ufs_desc->product, RPMB_CID_SIZE);
		info->rpmb_size_mult = ufs_desc->bRPMBRegion0Size;
		info->rel_wr_sec_c = 0 /*ufs_desc->bRPMB_ReadWriteSize*/;
		info->ret_code = RPMB_CMD_GET_DEV_INFO_RET_OK;
	} else {
...
static struct udevice *get_ufs(struct optee_private *priv, int dev_id)
{
	...
	if (priv->rpmb_mmc != NULL)
	{
		ufs = NULL;
	}
	else {
		if ((priv->rpmb_ufs != NULL)&& (priv->rpmb_dev_id == dev_id))
		{
			ufs = priv->rpmb_ufs;
		}
		else {
			...
			ufs = find_scsi_device(UFS_DEV_ID);
			if (ufs == NULL) {
				debug("Cannot find RPMB device of UFS\n");
			}
			else {
				priv->rpmb_ufs = ufs;
				priv->rpmb_dev_id = dev_id;
...
#elif defined(CONFIG_SUPPORT_UFS_RPMB)
		ufs = get_ufs(priv, (int)sreq->dev_id);
		if (ufs == NULL) {
			return TEE_ERROR_ITEM_NOT_FOUND;
		}
