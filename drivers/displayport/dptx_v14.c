...
static int32_t dptx_v14_alloc_mem(struct Dptx_Params **ppstDptx)
{
	...
	/* coverity[misra_c_2012_rule_21_3] */
...
static int32_t dptx_v14_init_params(const struct Dptx_Init_Params *pstInit_Params, struct Dptx_Params **ppstDptx)
{
	int32_t iRetVal = DPTX_RETURN_NO_ERROR;
	struct Dptx_Params *pstDptx = NULL;
	...
	(void)memcpy(pstDptx->aucMuxId, pstInit_Params->aucMux_Id, sizeof(uint8_t) * (size_t)PHY_INPUT_STREAM_MAX);
	(void)memcpy(pstDptx->aucVCP_Id, pstInit_Params->aucVcp_Id, sizeof(uint8_t) * (size_t)PHY_INPUT_STREAM_MAX);
	(void)memcpy(pstDptx->auiVIC, pstInit_Params->auiVIC, sizeof(uint32_t) * (size_t)PHY_INPUT_STREAM_MAX);
...
int32_t Dptx_V14_Init(const struct Dptx_Init_Params *pstDptx_Init_Params)
{
	int32_t iRetVal = DPTX_RETURN_NO_ERROR;
	struct Dptx_Params *pstDptx = NULL;
	...
	(void)Dptx_Vidin_Init(pstDptx, pstDptx_Init_Params->ucPixelEn_Type);
...
void Dptx_V14_Free_Handle(struct Dptx_Params       *pstDptx_Handle)
{
	if (pstDptx_Handle != NULL) {
		if (pstDptx_Handle->pucEdidBuf != NULL) {
			/* coverity[misra_c_2012_rule_12_3] */
