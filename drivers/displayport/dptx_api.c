...
int32_t Dpv14_Tx_API_Init(const struct dptx_api_init_params *pstdptx_api_init_params)
{
	...
	stDptx_Init_Params.ucMaxRate = (uint8_t)pstdptx_api_init_params->eMaxRate;
	stDptx_Init_Params.ucMaxLanes = (uint8_t)pstdptx_api_init_params->eMaxLanes;
	stDptx_Init_Params.ucPixelEn_Type = (uint8_t)pstdptx_api_init_params->eVidEncodingType;
	stDptx_Init_Params.ePhy_Dev_model = (enum PHY_DEVICE_MODEL)pstdptx_api_init_params->ucphy_model;

	(void)memcpy(stDptx_Init_Params.aucMux_Id, pstdptx_api_init_params->ucmux_id, sizeof(uint8_t) * (size_t)DPTX_INPUT_STREAM_MAX);
	(void)memcpy(stDptx_Init_Params.aucVcp_Id, pstdptx_api_init_params->ucvcp_id, sizeof(uint8_t) * (size_t)DPTX_INPUT_STREAM_MAX);
	(void)memcpy(stDptx_Init_Params.auiVIC, pstdptx_api_init_params->uiVIC, sizeof(uint32_t) * (size_t)DPTX_INPUT_STREAM_MAX);
...
int32_t Dpv14_Tx_API_Start(uint8_t ucNum_Of_Dps, uint32_t auiPixel_Clk[DPTX_INPUT_STREAM_MAX])
{
	...
	for (uiHDP_CheckTry = 0; uiHDP_CheckTry < (uint32_t)MAX_CHECK_HPD_NUM; uiHDP_CheckTry++) {
		...
		if ((iRetVal == DPTX_RETURN_NO_ERROR) && (ucHPD_Status == (uint8_t)HPD_STATUS_PLUGGED)) {
...
int32_t Dpv14_Tx_API_Perform_HPD_WorkingFlow(void)
{
	...
	iRetVal = dptx_reset_link_rate_and_lanes(pstHandle,
						 pstHandle->ucMax_Rate,
						 pstHandle->ucMax_Lanes);
	if (iRetVal != DPTX_RETURN_NO_ERROR) {
		dptx_err("Failed dptx_reset_link_rate_and_lanes");
		/* coverity[misra_c_2012_rule_15_1] */
		goto return_funcs;
	}
	iRetVal = Dptx_Intr_Handle_Hotplug(pstHandle);
