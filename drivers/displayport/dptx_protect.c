...
int32_t Dptx_Protect_Set_PW(struct Dptx_Params *pstDptx)
{
	...
	if (!bOffsetInRange) {
		...
		uiReg_Offset = (uint32_t)(pstDptx->uiProtect_RegAddr_Offset + (uint32_t)DP_PORTECT_CFG_PW_OK);
...
int32_t Dptx_Protect_Set_CfgLock(struct Dptx_Params *pstDptx, bool bLocked)
{
	...
	if (!bOffsetInRange) {
		...
		uiReg_Offset = (uint32_t)(pstDptx->uiProtect_RegAddr_Offset + (uint32_t)DP_PORTECT_CFG_LOCK);
...
int32_t Dptx_Protect_Set_CfgAccess(struct Dptx_Params *pstDptx, bool bAccessible)
{
	...
	if (!bOffsetInRange) {
		...
		uiReg_Offset = (uint32_t)(pstDptx->uiProtect_RegAddr_Offset + (uint32_t)DP_PORTECT_CFG_ACCESS);
