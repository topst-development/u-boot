...
#define DOS_PART_DEFAULT_SECTOR 4096
#else
...
#if CONFIG_IS_ENABLED(UFS)
	ALLOC_ALIGN_BUFFER(legacy_mbr, mbr,
			DIV_ROUND_UP(dev_desc->blksz, sizeof(legacy_mbr)), 4096);
#else
...
#else
#if CONFIG_IS_ENABLED(UFS)
	ALLOC_ALIGN_BUFFER((unsigned char, buffer, dev_desc->blksz, 4096);
#else
